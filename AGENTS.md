...
| `charts/runtime-conditions-crd/` | Helm chart wrapping the CRD (+ the same RBAC convenience roles, toggled by `rbac.create`). Published as an OCI artifact to GHCR on push to main. `crds/*.yaml` here is a copy kept in sync by `make manifests` - don't hand-edit it. |
